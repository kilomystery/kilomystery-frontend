...
const COMING_COPY: Record<Lang, ComingCopyPerLang> = {
  it: {
    ...
    heroSubtitle:
      "Stiamo preparando i nuovi lotti, i prossimi pop-up e qualche sorpresa extra per chi ama pesare il mistero e spacchettare la sorpresa.",
    bullet1:
      "♻️ Seconda vita ai pacchi – meno sprechi, più storie da spacchettare",
    bullet2:
      "📦 Lotti tracciati – ID lotto, pesi chiari, zero fuffa",
    bullet3:
      "🌍 Spedizione in tutta Europa – mystery box al kg anche fuori Italia",
    ...
    newsletterButton: "Avvisami al lancio",
    footerNote:
      "Nessuno spam. Solo news sui prossimi lotti, drop e pop-up.",
  },

  en: {
    badge: "Coming soon",
    heroTitle:
      "The next wave of mystery boxes by the kilo is coming 🔥",
    heroSubtitle:
  ...
  es: {
    ...
    bullet1:
      "♻️ Segunda vida para los paquetes – menos residuos, más historias que desempaquetar",
    bullet2:
      "📦 Lotes trazables – ID de lote, pesos claros, cero humo",
    bullet3:
      "🌍 Envíos a toda Europa – mystery boxes al kilo más allá de Italia",
    ...
    sustainTitle:
      "Segunda vida para los paquetes, menos residuos ♻️",
    sustainText:
      "Rescatamos paquetes perdidos, devoluciones no reclamadas y stock parado. En lugar de convertirse en basura, vuelven a circular como unboxings sorpresa. Cada box es un pequeño “no” al despilfarro y un “sí” a un consumo más consciente.",
    newsletterLabel:
  ...
  fr: {
    ...
    bullet2:
      "📦 Lots traçables – ID de lot, poids clairs, zéro blabla",
    bullet3:
      "🌍 Expédition dans toute l’Europe – des mystery box au kilo au-delà de l’Italie",
    comingTitle: "Ce que nous préparons",
    comingItem1:
      "De nouveaux lots Standard et Premium avec une sélection améliorée",
    comingItem2:
      "Un calendrier mis à jour de nos événements pop-up et corners physiques",
    comingItem3:
      "Des expériences supplémentaires pour les plus grosses commandes (roues, jeux, etc.)",
    comingItem4:
      "Davantage de transparence sur le CO₂ évité, les lots et leur origine",
    sustainTitle:
  ...
  de: {
    ...
    bullet1:
      "♻️ Zweites Leben für Pakete – weniger Müll, mehr Geschichten beim Unboxing",
    bullet2:
      "📦 Rückverfolgbare Chargen – Chargen-ID, klare Gewichte, kein Bullshit",
    bullet3:
      "🌍 Versand in ganz Europa – Mystery-Boxen pro Kilo auch außerhalb Italiens",
    ...
    sustainTitle:
      "Zweites Leben für Pakete, weniger Abfall ♻️",
    sustainText:
      "Wir retten verlorene Pakete, nicht abgeholte Retouren und liegengebliebene Lagerbestände. Statt im Müll zu landen, werden sie zu Überraschungs-Unboxings. Jede Box ist ein kleines „Nein“ zur Verschwendung und ein „Ja“ zu bewussterem Konsum.",
    newsletterLabel:
...
// NOINDEX per non rovinare la SEO
...
export default function ComingSoonPage({ params }: PageProps) {
  ...
  const t = COMING_COPY[lang] ?? COMING_COPY.it;

  return (
    <main className="container space-y-10 py-10 md:py-16">
      {/* HERO stile home */}
      <section className="rounded-2xl border border-white/10 bg-gradient-to-b from-white/5 to-transparent p-6 pt-10 md:p-10 md:pt-12">
        {/* logo + badge */}
        <div className="mb-6 flex items-center justify-between gap-4">
          ...
          <span className="inline-flex items-center rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-white/80 border border-white/20">
            {t.badge}
          </span>
        </div>

        {/* titolo gradient come home */}
        ...
        {/* newsletter-style form (solo UI) */}
        <div className="mx-auto mt-8 max-w-xl rounded-2xl border border-white/15 bg-black/20 p-4 backdrop-blur">
          ...
          <div className="mt-3 flex flex-col gap-3 sm:flex-row">
            <input
              ...
              className="flex-1 rounded-full border border-white/30 bg-black/40 px-4 py-2 text-sm text-white placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-[#7A20FF]"
            />
            <button
              type="button"
              className="inline-flex items-center justify-center rounded-full px-5 py-2 text-sm font-extrabold text-[#0c0f10]
                         bg-gradient-to-r from-[#7A20FF] to-[#20D27A]
                         shadow-[0_10px_28px_rgba(122,32,255,.25),0_6px_20px_rgba(32,210,122,.25)]
      ...
      {/* card in stile home: cosa stiamo preparando + sostenibilità */}
      <section className="grid gap-4 md:grid-cols-2">
        <div className="card space-y-3">
          <h2 className="text-lg md:text-xl font-extrabold">
            {t.comingTitle}
          </h2>
          <ul className="list-disc ps-5 space-y-1 text-sm text-white/80">
            <li>{t.comingItem1}</li>
            <li>{t.comingItem2}</li>
            <li>{t.comingItem3}</li>
            <li>{t.comingItem4}</li>
